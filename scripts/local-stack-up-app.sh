...
network_name="$(read_env_var_from_file "$APP_ENV_FILE" "GP_SHARED_NETWORK")"
if [ -z "$network_name" ]; then
  echo "GP_SHARED_NETWORK is required in $APP_ENV_FILE" >&2
  exit 1
fi
...
db_name="$(read_env_var_from_file "$APP_ENV_FILE" "DB_NAME")"
db_user="$(read_env_var_from_file "$APP_ENV_FILE" "DB_USER")"
...
if [ -z "$db_name" ] || [ -z "$db_user" ]; then
  echo "DB_NAME and DB_USER are required in $APP_ENV_FILE" >&2
  exit 1
fi

unset_compose_shell_overrides "$APP_ENV_FILE"
if [ "$openbao_secret_path" != "gpool" ]; then
  echo "OPENBAO_SECRET_PATH must be gpool in $APP_ENV_FILE (got $openbao_secret_path)" >&2
  exit 1
fi
